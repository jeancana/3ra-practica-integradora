...
import { uploader } from '../uploader.js'// Importamos el uploader para poder trabajar con Multer y subir archivos 
import { passportCall } from '../utils.js'// Manejo de JWT y Intercepción de errores para Modulo passport
import { authorizationMid } from '../utils.js'// Manejo de Autorización por ROLES 
import { ProductsController } from '../controllers/product.controller.mdb.js' // para trabajar con el Service

// Importando el Diccionario de Errores 
import errorsDictionary from '../services/error.dictionary.js';

// Paso 2: Generando una nueva Intanscia aca del ProductsController - Servicios Enlazados con MongoDB
const controller = new ProductsController()

// Inicializando el Router de Express
const router = Router()

// Para Chequear quien creo el producto
...
// *** 5) Endpoint para Generar 100 Productos con faker-js
...
router.get('/', passportCall('jwtAuth', { session: false }), authorizationMid(['user']), async (req, res) => {
...
router.get('/:pid', passportCall('jwtAuth', { session: false }), authorizationMid(['admin']), async (req, res) => {
