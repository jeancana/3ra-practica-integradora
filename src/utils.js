...
export const
    ...
    return async (req, res, next) => {
        ...
        res.status(403).send({ status: 'ERR', data: 'Sin permisos suficientess' });
...
// 3) Creamos un middleware que inyectaremos en el endpoint /api/sessions/restore 
...
export const restoreRequest = () => {
    ...
    return async (req, res, next) => {
        ...
        try {
            ...
            const token = generateToken({ email: email }, '30m'); // Válido solo por 5 mins
...
// 4) Creamos un middleware para Registrar un Usuarios 
...
// 4) Creamos un middleware para buscar User en la BD 
